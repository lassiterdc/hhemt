...
harness: claude-code
next_phase: '4'
next_phase_title: bundle_report_data() method + opt-in invocation contract
next_phase_doc: /home/dcl3nd/dev/agentic-workspace/library/docs/planning/projects/TRITON-SWMM_toolkit/features/render_bundle/4 bundlereportdata method optin invocation contract.md
leave_off_step: Phase 3 complete and pushed; entering Phase 4 (bundle_report_data() method + opt-in invocation contract)
...
- Leave-off step: Phase 3 complete and pushed; entering Phase 4 (bundle_report_data() method + opt-in invocation contract)
- Next phase: Phase 4 — bundle_report_data() method + opt-in invocation contract
- Next phase doc: `/home/dcl3nd/dev/agentic-workspace/library/docs/planning/projects/TRITON-SWMM_toolkit/features/render_bundle/4 bundlereportdata method optin invocation contract.md`
...
1. Sidecar scratch doc:
   `/home/dcl3nd/dev/TRITON-SWMM_toolkit/.claude/worktrees/toolkit_05-03_1359_render-bundle-pwi/sidecars/sidecar_phase4_2026-05-04_1015.md`

2. Next phase doc:
   `/home/dcl3nd/dev/agentic-workspace/library/docs/planning/projects/TRITON-SWMM_toolkit/features/render_bundle/4 bundlereportdata method optin invocation contract.md`
...
- **Phase**: 4 — bundle_report_data() method + opt-in invocation contract
- **Heaviness**: heavy
- **Session gate (after completion)**: true
- **Dependencies**: library/docs/planning/projects/TRITON-SWMM_toolkit/features/render_bundle/3 manifestharvest infrastructure renderer audit stipulation.md (now in implemented/)
...
The prior session halted at: `Phase 3 complete and pushed; entering Phase 4 (bundle_report_data() method + opt-in invocation contract)`
...
`Checkpoint-match assertion: Sidecar: sidecar_phase4_2026-05-04_1015.md. Leave-off step (from
checkpoint): Phase 3 complete and pushed; entering Phase 4 (bundle_report_data() method + opt-in invocation contract). Next phase: Phase 4 — bundle_report_data() method + opt-in invocation contract.
...
- **Goal**: collapse all hardcoded string literals across the toolkit codebase to a single (or very small number of) source-of-truth surfaces; all other references become class-object reads. Phased approach:
  - **Phase 1 — B1 (in-flight, this PWI's Phase 6)**: fix the 2 HIGH-priority literals (`"weather_timeseries.nc"` → `cfg_analysis.weather_timeseries`; `"_version.json"` → `VERSION_FILE_NAME`).
  - **Phase 2 — B2 (out-of-scope-for-now)**: capture all MEDIUM-priority literals where a path attribute already exists (`"sims/{event_id}/swmm/hydraulics.inp"` → `scenario_paths.swmm_hydraulics_inp` after attribute-name verification; `"scenario_status.csv"` references → `analysis.analysis_paths.scenario_status_csv` if present).
  - **Phase 3 — B3 (out-of-scope-for-now)**: refactor AnalysisPaths from dataclass to property-based class so leaf filenames (`"analysis_datatree.zarr"`, `"sensitivity_datatree.zarr"`) are config-derivable. Cross-cutting refactor that touches every AnalysisPaths constructor + every test that mocks it.
  - **Phase 4 — Snakemake template chunks**: centralize the Snakemake rule-shell-command templates currently embedded in `src/TRITON_SWMM_toolkit/workflow.py` (multiple multi-hundred-line embedded strings). Extracting these to a templates module would make `workflow.py` MASSIVELY more readable AND abstract patterns potentially reusable by other tool sets that emit Snakemake rules.
  - **Phase 5 — `.value` + `.definition` attribute pattern**: introduce a self-documenting-constant idiom — e.g., `class StatusFlag: e_consolidate_complete = SelfDocConstant(value="e_consolidate_complete", definition="Sentinel flag written by Snakemake's consolidate rule on successful per-mode flat-zarr emission.")`. A future maintainer reading `StatusFlag.e_consolidate_complete.definition` learns the meaning without grepping. Aligns with self-documenting-enum patterns common in larger systems (Pydantic `Field(..., description=...)`, Python `enum.Enum` with docstrings per value). Novel for this codebase per investigation Q5.
- **Why this is high-leverage**: removes the grep-to-understand-a-constant friction; surfaces drift (a hardcoded literal that doesn't match the canonical source becomes a fail-loud signal rather than silently working until something breaks); creates a reusable pattern for adjacent toolkits (multidriver-swg, hydro-fetch).
- **Recommended promotion**: `/plan-implementation` after this PWI closes; multi-phase plan structure aligned with the 5-phase rollout above. Each phase is independently committable; full rollout can span multiple sessions.

###### Plan-review-surfaced follow-ups (Phase 6 baseline-repair plan, partitioned-cuddling-aho.md)

- **HTML-renderer manifest-sidecar emission (SE F-FU Flag 7)** — `errors_and_warnings.py` and `scenario_status_appendix.py` are pure HTML/text renderers that end with `output_path.write_text(html)` and emit no `*.manifest.json` sidecar. Phase 6's baseline-repair plan adds `with prov.artist(...)` AST blocks to satisfy provenance discipline tests but does not wire them through any emission helper. Follow-up: extend `emit_plot_with_sources` to accept HTML payloads, OR factor out a parallel `emit_html_with_sources` so HTML renderers emit manifest sidecars matching the matplotlib renderers' pattern. Would let the bundle harvest declare HTML renderer outputs uniformly.
- **`compilation_cpu_successful` property root-cause investigation (SE F-FU Flag 8 + TS toolkit specialist refinement)** — Cluster B's session-scoped fixture `tritonswmm_cpu_compiled` is a workaround for `prepare_scenario`'s compile gate failing despite a valid on-disk build. The plan's stated root cause (fresh init = empty log = property false) may not be accurate: per `system.py:1285-1294`, the property already calls `retrieve_compilation_log("cpu")` which reads from disk. The actual cause may be a path-resolution bug in `retrieve_compilation_log` for the synth-fixture's cache layout, OR an interaction with the synth fixture's `_software_root`-vs-`system_directory` split. Fixture works regardless of root cause. Investigate whether the fixture is necessary or whether a path-resolution fix would obviate it. If a fix is found, the fixture can be removed.
