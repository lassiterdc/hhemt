...
next_phase_doc: /home/***REMOVED***/dev/agentic-workspace/library/docs/planning/projects/TRITON-SWMM_toolkit/features/per_sub_analysis_system_configs/3 snakemake workflow generation.md
...
- Next phase doc: `/home/***REMOVED***/dev/agentic-workspace/library/docs/planning/projects/TRITON-SWMM_toolkit/features/per_sub_analysis_system_configs/3 snakemake workflow generation.md`
...
1. Sidecar scratch doc:
   `/home/***REMOVED***/dev/TRITON-SWMM_toolkit/.claude/worktrees/toolkit_05-15_0904_per-sub-analysis-system-configs-pwi/sidecars/sidecar_phase3_2026-05-15_1759.md`

2. Next phase doc:
   `/home/***REMOVED***/dev/agentic-workspace/library/docs/planning/projects/TRITON-SWMM_toolkit/features/per_sub_analysis_system_configs/3 snakemake workflow generation.md`
...
| 1 | `src/TRITON_SWMM_toolkit/sensitivity_analysis.py` (+132/-3); `tests/test_PC_07_per_sa_system_configs.py` (new) | 0 (clean — exit 0, no violations) | N/A — gate cleared. Tests: `tests/test_PC_07_per_sa_system_configs.py` 6/6 pass; `tests/test_synth_05_sensitivity_analysis_with_snakemake.py` fast tier 4/4 pass as backward-compat proxy for PC_05 (real Norfolk data unavailable in this worktree). |
| 2 | `src/TRITON_SWMM_toolkit/sensitivity_analysis.py` (+40/-4); `tests/test_PC_07_per_sa_system_configs.py` (+58/-0) | 0 (clean — exit 0, no violations) | N/A — gate cleared. Tests: `tests/test_PC_07_per_sa_system_configs.py` 8/8 pass (incl. 2 new Phase 2 tests); `test_synth_05_sensitivity_analysis_with_snakemake.py` fast 4/4 pass (PC_05 proxy); `test_synth_04_multisim_with_snakemake.py` fast 11 passed + 1 slow-skipped (PC_04 proxy). Real-data PC_04/PC_05 unavailable in worktree. |
| 3 | `src/TRITON_SWMM_toolkit/workflow.py` (+~100/-~22); `tests/test_PC_07_per_sa_system_configs.py` (+52); `tests/test_synth_05_sensitivity_analysis_with_snakemake.py` (+37); `tests/fixtures/golden_snakefiles/multi_sim.Snakefile.golden` (regenerated); `tests/fixtures/golden_snakefiles/sensitivity_master.Snakefile.golden` (regenerated) | 0 (clean — exit 0, no violations) | N/A — gate cleared. Tests: `tests/test_PC_07_per_sa_system_configs.py` 11/11 pass (incl. 3 new Phase 3 unit tests); `tests/test_synth_05_sensitivity_analysis_with_snakemake.py` fast 5/5 pass (incl. new `test_phase3_master_snakefile_emits_per_target_setup_rules`); `tests/test_workflow_snakefile_byte_identity.py` 2/2 pass (both goldens regenerated to capture Phase 3 structural changes AND pre-existing worktree-path drift — see follow-up); `tests/test_synth_04_multisim_with_snakemake.py` non-slow tier 11/12 (1 slow-skipped) as PC_04 proxy. Real-data PC_04/PC_05 unavailable in worktree. Cumulative fast-tier across PC_07 + byte-identity + synth_04 + synth_05: 29 passed, 1 skipped, 5 deselected. |
%%
...
###### Snakefile-golden byte-identity test brittleness across worktrees (Phase 3 discovery)

**Symptom**: `tests/test_workflow_snakefile_byte_identity.py` is structurally fragile because the captured golden Snakefiles embed absolute paths that include the worktree subdirectory (e.g., `/home/***REMOVED***/dev/TRITON-SWMM_toolkit/.claude/worktrees/<slug>/test_data/...`). Every new worktree changes the embedded paths, breaking byte-identity even when no source-side change occurred. Discovered Phase 3 when the test failed against pre-existing path drift from the prior `toolkit_05-14_1432_plotly-html-extension-pwi` worktree where the goldens were last captured (commit `39c0fa6`). Phase 3 also adds structural drift (intentional `rule setup` → `rule setup_target_0` rename + per-target flag rename), so the goldens were regenerated against this worktree's paths.

**Concrete failure surface**: The test is by-design red on every new worktree until someone regenerates the goldens. The regeneration must be invoked with the worktree's `src/` prepended to PYTHONPATH (matching `conftest.py`'s Layer 1 sys.path-prepend guard), otherwise the goldens are written from the conda env's main-tree editable install and silently capture the OLD source. Hit this bug live in Phase 3 — the first golden regenerate wrote stale content because `conda run python -c '...'` bypassed the conftest guard.

**Proposed fix directions** (capture for triage, not commit):
1. **Path normalization in test**: rewrite paths in both golden and emitted content with a placeholder (e.g., `{WORKTREE_PATH}`) before comparison, or strip worktree-prefix from both sides.
2. **Path normalization in builder**: have the Snakefile generator emit paths relative to the analysis directory (not absolute), so worktree-prefix doesn't appear in output.
3. **Document regen invocation**: add a `make regenerate-goldens` or `scripts/regenerate_snakefile_goldens.py` target that sets PYTHONPATH correctly, so future goldening doesn't repeat the conftest-bypass footgun.
4. **xfail until fixed**: mark byte-identity tests `xfail` until one of the above lands.

**Related**: the byte-identity test docstring says "If byte-identity fails, the source-side Snakefile generation introduced silent drift and must be fixed before merge — NOT papered over by regenerating the golden." This phrasing conflicts with the de facto practice (regenerate on every intentional source change, including Phase 5 baseline at commit `db1d23b` and Phase 3 here). The docstring's "NOT papered over" caution is correct only for silent drift; explicit structural changes legitimately require golden regeneration. The docstring may want refinement to distinguish.

###### Phase 3 closeout HAND-OFF: AW-side bookkeeping blocked by cross-session orphan-staging in $AGENTIC_WORKSPACE

**Halt state**: At Phase 3 commit time (2026-05-15T18:55), `scripts.utils.git_stage --commit` against $AGENTIC_WORKSPACE failed with **exit 11 IndexNotEmpty**. The shared index contained **24 staged files** from a parallel/prior session (PWI ecosystem improvements — pwi atomic/common-prefix/multiphase paths, qaqc SKILL, address-smell, capture-idea, invoke-specialist, plan-implementation, review-followup-ideas, wrapup-session, push approval gate, pwi specialist roll in evaluation; 2 new decision docs (fidelity gate tripwires, pwi gate suppression); .claude/settings.json; 7 renames moving bundle_portable_report_regeneration to completed/; phase-doc moves: invoke_specialist_plan_purpose_authoring_guardrails phase 3 → implemented/, pwi_resume_smell_resolution phase 1 → implemented/). Per the protocol's Step 4e ("the foreign staged files represent another session's intent; silently resetting them discards that intent"), user-directed disposition: **Option B (hand-off)**.

**What landed durably**:
- **Toolkit cwd-bucket commit** `b223cca feat(sensitivity-analysis-orchestration): emit per-target setup rules in sensitivity Snakefile` — pushed to origin worktree branch `worktree-toolkit_05-15_0904_per-sub-analysis-system-configs-pwi` at 18:56. Phase 3 source code + tests + regenerated goldens are durable on remote. Worktree branch is one commit ahead of the rebaseline parent `5c8fd63` (Phase 2 audit-row commit).

**What remains for a future session** (the bookkeeping the autonomous phase loop normally does at steps 6-7 of Per-Phase QAQC and Commit):
1. **Resolve the AW orphan-staging first**: another session committed those 24 files (or migrated them via the migration tool) before this hand-off can resume. Until that resolves, no AW-side commit is possible from this worktree.
2. **AW-side aw-bucket commit**: tick the Phase 3 DoD checkboxes in `library/docs/planning/projects/TRITON-SWMM_toolkit/features/per_sub_analysis_system_configs/3 snakemake workflow generation.md`. The DoD checkbox edits are already on disk in agentic-workspace (uncommitted modifications); no re-edit needed. Suggested commit: `chore(planning-infrastructure): tick Phase 3 DoD checkboxes in per_sub_analysis_system_configs`.
3. **Move phase doc to implemented/**: `conda run -n agentic --cwd $AGENTIC_WORKSPACE python -m scripts.generate.complete_plan --move-phase "$AGENTIC_WORKSPACE/library/docs/planning/projects/TRITON-SWMM_toolkit/features/per_sub_analysis_system_configs/3 snakemake workflow generation.md"`
4. **Update master Phase Status table** at `$AGENTIC_WORKSPACE/library/docs/planning/projects/TRITON-SWMM_toolkit/features/per_sub_analysis_system_configs/per sub analysis system configs master.md` — flip Phase 3 row to `completed` and update the doc link to `implemented/3 snakemake workflow generation.md`. NOTE: the master doc currently has pre-existing `implemented/implemented/implemented/` path-prefix accumulation (visible in `git -C $AGENTIC_WORKSPACE diff`); see whether the next agent should fix that pre-existing drift simultaneously or leave it untouched (it predates Phase 3).
5. **Sidecar sync-back**: after the AW-side commits land, run `scripts/generate/write_compaction_sidecar.py --mode sync-back` to flush this sidecar's `### Phase Audit History` (now containing Phase 1, 2, AND 3 rows) and `# Follow-up Ideas` back to main scratch.
6. **Phase 4 resume**: the autonomous phase loop's next iteration enters Phase 4 (Validation and testing) — `$AGENTIC_WORKSPACE/library/docs/planning/projects/TRITON-SWMM_toolkit/features/per_sub_analysis_system_configs/4 validation and testing.md`. Phase 4's heaviness is unknown to this session; the session capacity gate fires there.

**Cross-session hygiene observation**: Two parallel sessions both targeting AW prompt-doc and planning-doc state is the canonical orphan-staging hazard the protocol exists to catch. The 2026-05-02 reference in commit SKILL Step 4e — "the foreign staged files represent another session's intent; silently resetting them discards that intent" — fires here in Option B form. Worth assessing whether the agentic-workspace stash/staging coordination needs a separate enhancement to alert sessions earlier (e.g., a SessionStart-time warning when `git diff --cached` is non-empty on session start). Captured as a separate idea-worthy item.
