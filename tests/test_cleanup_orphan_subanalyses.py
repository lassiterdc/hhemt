...
@pytest.fixture
def sa_with_scratch_analyses(norfolk_sensitivity_analysis_cached, tmp_path, monkeypatch):
    ...
    for sa_id in sa.df_setup.index.astype(str):
        (scratch / f"{sa.analyses_prefix}{sa_id}").mkdir()
...
def _make_orphan_dir(sensitivity_analysis, sa_id: str) -> Path:
    ...
    orphan = sensitivity_analysis.analyses_dir / f"sa_{sa_id}"
    orphan.mkdir(parents=True, exist_ok=True)
    (orphan / "dummy.txt").write_text("stale content")
    return orphan


def test_find_orphans_empty_when_csv_matches_disk(sa_with_scratch_analyses):
    sa = sa_with_scratch_analyses
    orphans = sa.find_orphan_analysis_dirs()
    assert orphans == []


def test_find_orphans_detects_extra_dir(sa_with_scratch_analyses):
    sa = sa_with_scratch_analyses
...
def test_cleanup_dry_run_does_not_delete(sa_with_scratch_analyses):
    sa = sa_with_scratch_analyses
    orphan = _make_orphan_dir(sa, "99_dry")
    result = sa.cleanup_orphan_analysis_dirs(dry_run=True, force=False, verbose=False)
    assert orphan in result
    assert orphan.exists(), "Dry-run must not delete"


def test_cleanup_apply_without_force_raises(sa_with_scratch_analyses):
    sa = sa_with_scratch_analyses
    orphan = _make_orphan_dir(sa, "99_noforce")
    with pytest.raises(ValueError, match="force=True"):
        sa.cleanup_orphan_analysis_dirs(dry_run=False, force=False, verbose=False)
    assert orphan.exists(), "Must not delete without force"


def test_cleanup_apply_with_force_deletes(sa_with_scratch_analyses):
    sa = sa_with_scratch_analyses
    orphan = _make_orphan_dir(sa, "99_apply")
    result = sa.cleanup_orphan_analysis_dirs(dry_run=False, force=True, verbose=False)
    assert orphan in result
    assert not orphan.exists(), "Apply+force must delete"
    for sa_id in sa.df_setup.index.astype(str):
        assert (sa.analyses_dir / f"{sa.analyses_prefix}{sa_id}").exists()


def test_non_sa_prefix_dirs_ignored(sa_with_scratch_analyses):
    sa = sa_with_scratch_analyses
    non_sa = sa.analyses_dir / "other_dir"
    non_sa.mkdir(parents=True, exist_ok=True)
    orphans = sa.find_orphan_analysis_dirs()
    assert non_sa not in orphans


def test_sa_prefix_with_invalid_charset_not_orphaned(sa_with_scratch_analyses):
    ...
    by --apply --force."""
    sa = sa_with_scratch_analyses
    hostile = sa.analyses_dir / "sa_has spaces"
    hostile.mkdir(parents=True, exist_ok=True)
    orphans = sa.find_orphan_analysis_dirs()
    assert hostile not in orphans


def test_missing_analyses_dir_is_noop(norfolk_sensitivity_analysis_cached, monkeypatch, tmp_path):
    sa = norfolk_sensitivity_analysis_cached.sensitivity
