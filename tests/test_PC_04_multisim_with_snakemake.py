...
def test_validate_outputs_against_reference(norfolk_multi_sim_analysis):
    ...
    """
    Validate freshly-generated outputs against reference data.

    Ensures that memory optimizations (Phase 1.1, 1.3) do not change output values.
    Compares all 8 consolidated zarr stores and 16 per-simulation zarr stores
    against pre-recorded reference data.

    Reference data location: test_data/reference_for_addressing_memory_allocation_issues/

    Note: Performance timer variables are skipped (non-deterministic).

    This test runs the full workflow first to generate fresh outputs, then validates
    them against the reference data.
    """
    import xarray as xr
    ...
    # Run the workflow to generate fresh outputs
    print("\n[Workflow] Running Snakemake workflow to generate outputs...")
    result = analysis.submit_workflow(
        mode="local",
        process_system_level_inputs=True,
        overwrite_system_inputs=False,
    ...
    # Ensure all scenarios are processed
    tst_ut.assert_analysis_workflow_completed_successfully(analysis)

    # Note: The Snakemake workflow with process_timeseries=True should have processed outputs,
    # but we verify by checking if outputs exist. If not, we process them here.
    enabled_models = tst_ut.get_enabled_model_types(analysis)

    # Check if any processed outputs exist (just check if sims/*/processed has zarr files)
    sims_dir = analysis.analysis_paths.analysis_dir / "sims"
    has_processed_outputs = False
    if sims_dir.exists():
        for scenario_dir in sims_dir.iterdir():
            if scenario_dir.is_dir():
                processed_dir = scenario_dir / "processed"
                if processed_dir.exists() and any(processed_dir.glob("*.zarr")):
                    has_processed_outputs = True
                    break

    if not has_processed_outputs:
        print("\n[Processing] Processing timeseries outputs for all scenarios...")
        for event_iloc in analysis.df_sims.index:
            proc = analysis._retrieve_sim_run_processing_object(event_iloc)
            print(f"  Processing scenario {event_iloc}...")
            for model_type in enabled_models:
        ...
        print("\n[Processing] Outputs already processed by workflow")

    # Consolidate analysis outputs (creates analysis-level zarr stores)
    print("\n[Consolidation] Generating analysis-level consolidated outputs...")
    analysis.consolidate_analysis_outputs(
        overwrite_outputs_if_already_created=True,
        verbose=True,
        compression_level=5,
    )
    ...
    ref_dir = (
        ...
        / "test_data"
        / "reference_for_addressing_memory_allocation_issues"
    )

    # Analysis-level consolidated outputs directory
    actual_dir = analysis.analysis_paths.analysis_dir

    # Define analysis-level consolidated files to compare
    analysis_level_files = [
    ...
    # Define per-simulation files to compare (scenario 0 only)
    per_simulation_files = [
        # Timeseries
        "TRITONSWMM_TRITON_tseries.zarr",
        "TRITONSWMM_SWMM_node_tseries.zarr",
        "TRITONSWMM_SWMM_link_tseries.zarr",
        "TRITONSWMM_perf_tseries.zarr",
        "TRITON_only_tseries.zarr",
        "TRITON_only_perf_tseries.zarr",
        "SWMM_only_node_tseries.zarr",
        "SWMM_only_link_tseries.zarr",
        # Summaries
        "TRITONSWMM_TRITON_summary.zarr",
        "TRITONSWMM_SWMM_node_summary.zarr",
        "TRITONSWMM_SWMM_link_summary.zarr",
        "TRITONSWMM_perf_summary.zarr",
        "TRITON_only_summary.zarr",
        "TRITON_only_perf_summary.zarr",
        "SWMM_only_node_summary.zarr",
        "SWMM_only_link_summary.zarr",
    ]

    # Performance timer variables to skip (vary by system load)
    performance_vars_to_skip = [
        "Compute",
        "IO",
        "Init",
        "MPI",
    ...
    print("\n" + "=" * 70)
    print("VALIDATING OUTPUTS AGAINST REFERENCE DATA")
    print("=" * 70)

    validation_results = []

    # Part 1: Validate analysis-level consolidated outputs
    print("\n--- PART 1: Analysis-Level Consolidated Outputs ---")
    for filename in analysis_level_files:
        ref_path = ref_dir / filename
        actual_path = actual_dir / filename

        # Check both files exist
        tst_ut.assert_file_exists(ref_path, f"Reference: {filename}")
        tst_ut.assert_file_exists(actual_path, f"Actual: {filename}")

        # Load datasets
        ds_ref = xr.open_dataset(ref_path, engine="zarr", chunks="auto")
        ds_actual = xr.open_dataset(actual_path, engine="zarr", chunks="auto")

        # Determine if this is a performance dataset
        is_performance = "performance" in filename.lower()
        skip_vars = performance_vars_to_skip if is_performance else []

        try:
            # Compare datasets
            tst_ut.assert_datasets_equal(
                ds_reference=ds_ref,
                ds_actual=ds_actual,
                dataset_name=filename,
                skip_vars=skip_vars,
            )
            validation_results.append((filename, "✅ PASS"))
            print(f"✅ {filename}: All values match")

        except AssertionError as e:
            validation_results.append((filename, f"❌ FAIL: {str(e)}"))
    ...
    # Part 2: Validate per-simulation outputs (scenario 0 only)
    print("\n--- PART 2: Per-Simulation Outputs (Scenario 0) ---")
    scenario_0_ref_dir = ref_dir / "sims" / "0-event_id.0" / "processed"
    scenario_0_actual_dir = actual_dir / "sims" / "0-event_id.0" / "processed"

    for filename in per_simulation_files:
        ref_path = scenario_0_ref_dir / filename
        actual_path = scenario_0_actual_dir / filename

        # Check both files exist
        tst_ut.assert_file_exists(ref_path, f"Reference (scenario 0): {filename}")
        tst_ut.assert_file_exists(actual_path, f"Actual (scenario 0): {filename}")
        ...
        # Determine if this is a performance dataset
        is_performance = "perf" in filename.lower()
        skip_vars = performance_vars_to_skip if is_performance else []
        ...
        try:
            ...
            tst_ut.assert_datasets_equal(
                ds_reference=ds_ref,
                ds_actual=ds_actual,
                dataset_name=f"sims/0-event_id.0/processed/{filename}",
                skip_vars=skip_vars,
            )
            validation_results.append((filename, "✅ PASS"))
            print(f"✅ {filename}: All values match")
    ...
    for filename, result in validation_results:
        print(
            f"  {result}: {filename}" if "PASS" in result else f"  {filename}: {result}"
        )

    passed = sum(1 for _, r in validation_results if "PASS" in r)
    total = len(validation_results)
    print(f"\nPassed: {passed}/{total}")

    if passed < total:
        pytest.fail(f"Output validation failed for {total - passed} dataset(s)")
