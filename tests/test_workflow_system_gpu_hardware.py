"""GRES regression check for per-member partition-derived GPU hardware.
...
member selects (R7). Phase 6 generalized this per-row: each
member's GRES substring is resolved from
...
rule's ``--gres`` substring mirrors each member's PARTITION-DERIVED GPU
...
def test_per_sub_partition_resolves_gres_hardware(synth_sensitivity_with_partition_axis):
    """Each member's GRES substring is its PARTITION-derived gpu_hardware.
    ...
    for member_id, sub in sensitivity.members.items():
        ...
        assert gpu_hw == "a6000", (
            f"member_id={member_id}: partition {partition!r} should derive gpu_hardware "
        ...
        assert (f"gpu:{gpu_hw}" in block) or (f'gpu_model="{gpu_hw}"' in block), (
            f"member_id={member_id}: partition-derived hardware {gpu_hw!r} not found in the "
