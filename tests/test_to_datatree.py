...
def test_to_datatree_skips_missing_modes(synth_all_models_analysis):
    """Absent consolidated outputs produce a tree with only the root node."""
    tree = synth_all_models_analysis.process.to_datatree()
    assert isinstance(tree, xr.DataTree)
    assert "analysis_id" in tree.attrs


def test_mode_to_tree_path_keys_match_mode_config(synth_all_models_analysis):
    """`_MODE_TO_TREE_PATH` must cover every `_MODE_CONFIG` key (schema SSOT)."""
    proc = synth_all_models_analysis.process
    assert set(proc._MODE_TO_TREE_PATH.keys()) == set(proc._MODE_CONFIG.keys())


def test_to_datatree_is_lazy_when_outputs_present(synth_all_models_analysis_cached):
    ...
    for node in populated:
        for var in node.ds.data_vars.values():
            assert hasattr(var.data, "dask"), (
                f"{node.path}/{var.name} is not dask-backed; to_datatree must open lazily."
            )
