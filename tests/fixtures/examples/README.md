...
## `sensitivity_per_member_system_configs_example.csv`

Demonstrates the optional `system_config_yaml` column in sensitivity-analysis
CSVs (added in the `per_member_system_configs` feature). Two distinct
...
members on two compile targets (rows 0+2 collapse onto the 10 m target;
...
1. Each referenced YAML exists on disk and passes Pydantic validation
   (`load_system_config()`).
2. Every member YAML's model-type toggles match the master system config.
