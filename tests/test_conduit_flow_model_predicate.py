...
site, ``_per_sim_per_sa_rule_all_inputs`` for enumeration), so nothing but a test that
...
def _conduit_counts(snakefile: str) -> tuple[int, int]:
    """(rule-definition count, rule-all entry count) for conduit_flow per-sa figures."""
    rule_defs = len(re.findall(r"^rule plot_per_sim_per_sa_conduit_flow:", snakefile, re.M))
    m = re.search(r"^rule all:\n    input:\n((?:        .*\n)+)", snakefile, re.M)
    rule_all_body = m.group(1) if m else ""
    rule_all_entries = rule_all_body.count("conduit_flow__sa.")
...
def test_conduit_flow_emission_and_enumeration_agree(
    ...
    # catches an over-firing gate that drops the whole per_sa selection rather than
    # the one SWMM-derived template.
    assert re.search(r"^rule plot_per_sim_per_sa_peak_flood_depth:", generated, re.M), (
