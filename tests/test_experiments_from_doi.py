...
# --- ADR-21: the reproducer's derived config carries sif_root, never a pointer -------------


def test_set_sif_root_writes_only_sif_root(tmp_path):
    from hhemt.utils import read_yaml

    src = tmp_path / "hpc.yaml"
    src.write_text("container:\n  sif_root: /orig/placeholder\n  gpu_flag: --nv\n")
    out = TRITON_SWMM_experiment._set_sif_root(
        src, sif_root=tmp_path / "sifs", target_path=tmp_path / "hpc.derived.yaml"
    )
    cfg = read_yaml(out)
    assert cfg["container"]["sif_root"] == str((tmp_path / "sifs").resolve())
    assert cfg["container"]["gpu_flag"] == "--nv"  # the rest of the block is preserved
    assert not {"sif_path", "sif_paths_by_arch", "sif_sha256"} & set(cfg["container"])
    assert src.read_text().count("/orig/placeholder") == 1  # the SOURCE config is never edited
