...
- [x] `override_hpc_total_nodes` added to both `submit_workflow()` methods
- [x] Early `ConfigurationError` guard added for wrong-mode usage
- [x] Override threaded through `_submit_single_job_workflow()` → `_validate_single_job_dry_run()` and `_generate_single_job_submission_script()`
- [x] `#SBATCH --nodes=` in generated script reflects the override value
