...
| # | Strength | Notes |
|---|----------|-------|
| 1 | **`philosophy.md` is a standalone file** | Separates "how we work" from "what the project does". CLAUDE.md stays focused on architecture; philosophy is invoked explicitly when needed. Also surfaces as a living document AI can be asked to check against. |
...
| 6 | **Terminology section in `philosophy.md`** | Precise vocabulary definitions prevent AI/human miscommunication. E.g., "compound" vs. "combined", `event_iloc`, `ss` vs. `ensemble`. TRITON-SWMM toolkit has no equivalent. |
...
|---------|-------|----------------|
| "Backward Compatibility" (with code example) | Good philosophy content, but wrong location. Should live in `philosophy.md`. The code example adds length without much value for an AI that already understands the concept. | Move to `philosophy.md`; keep a one-line summary in CLAUDE.md |
| "Completion Status: Log-Based Checks..." | Philosophy content, not architectural reference | Move to `philosophy.md` |
| "Logging & Error Handling" — Logging Patterns subsection | Duplicates patterns better discovered from code. The `print([NAMESPACE])` pattern is clear enough in code. | Trim to exception hierarchy only; move logging philosophy to `philosophy.md` |
...
|---|---------|---------------------|-------|
| 1 | `philosophy.md` as standalone file | `.prompts/philosophy.md` | Use ss-fha version as base; replace ss-fha-specific terminology with TRITON-SWMM equivalents. Merge in content extracted from CLAUDE.md (Table 2, item H). Avoid redundancy — when an existing section is related to something in ss-fha's philosophy, decide case-by-case whether to add, clarify, or skip. |
...
| 6 | Terminology section | Inside `.prompts/philosophy.md` | Replace ss-fha terminology with TRITON-SWMM equivalents. Confirmed terms to include: `model_type` (`triton`, `tritonswmm`, `swmm`) vs. `run_mode` (`serial`, `openmp`, `mpi`, `gpu`, `hybrid`) — these are frequently confused; `multi_sim_run_method` vs. `run_mode` — different axes of configuration; `event_iloc` — flat integer index (already in CLAUDE.md, migrates here); `in_slurm` — derived boolean, not just the env var. Scope to terms where misuse causes real errors, as in ss-fha. |
| 7 | "Most arguments should not have defaults" | Inside `.prompts/philosophy.md` | Add as new section. |
| 8 | ~~"No shims for poorly formatted inputs"~~ | ~~Inside `.prompts/philosophy.md`~~ | **Excluded** — no direct TRITON-SWMM equivalent at this time. The ss-fha version is specifically about HydroShare data formatting; this repo has no analogous concept. Revisit if a relevant pattern emerges. |
| 9 | "Avoid aliases" | Inside `.prompts/philosophy.md` | Add as clarification/extension to existing no-backward-compatibility section rather than a separate entry, unless the concept is meaningfully distinct. |
| 10 | "All variables/imports/args must be used" | Inside `.prompts/philosophy.md` | Add with investigation protocol intact. |
| 11 | "Functions have docstrings, type hints" | Inside `.prompts/philosophy.md` | Add as a rule. Apply organically going forward: `qaqc_and_commit.md` already prompts checking touched scripts against philosophy, so adherence will accumulate naturally without a global retrofit pass. |
| 12 | `#user:` blocking comment convention | `.prompts/implementation_plan.md` + `philosophy.md` | Bundled with items 4 & 5. |
...
| H | Extract philosophy content from CLAUDE.md into `philosophy.md` | CLAUDE.md shrinks; new `.prompts/philosophy.md` created | Cleaner separation of concerns | ✅ Yes (contingent on item 1 in Table 1) |x |
...
| M | Add "philosophy check" step to `doc_freshness_check.md` | `.prompts/doc_freshness_check.md` edit | Keeps philosophy.md current after changes | ✅ Yes (if philosophy.md adopted) |x |
...
| `doc_freshness_check.md` | Check if CLAUDE.md / agent docs need updates | No ss-fha equivalent; especially valuable given `.claude/agents/` system | TBD | Will need `philosophy.md` added to checklist (Table 2, item M) |
...
If `philosophy.md` is adopted (Table 1, item 1), these CLAUDE.md sections would move there:
...
├── philosophy.md                     # NEW: ss-fha base + TRITON-SWMM content
...
**Note on `qaqc_and_commit.md` and `philosophy.md` staying in sync**: No manual sync is needed. `qaqc_and_commit.md` instructs the AI to *read* `philosophy.md` at review time, so new philosophy rules are picked up automatically without any changes to `qaqc_and_commit.md`.
...
- [x] `philosophy.md` written: ss-fha base + TRITON-SWMM terminology + CLAUDE.md philosophy content extracted
...
- [x] Philosophy content removed from CLAUDE.md (Table 2, item H) with pointer to `philosophy.md`
...
- [x] `philosophy.md` added to Documentation Update Checklist in CLAUDE.md (Table 2, item M)
- [x] `philosophy.md` elevated to mandatory read: CLAUDE.md now instructs "Read `.prompts/philosophy.md` before beginning any task"
...
- [x] All philosophy.md additions: plan-deviation nuance, utility candidates rule, pyright/Pylance section, GIS data type preference, testing philosophy section
- [x] "Confirm before spawning subagents" rule removed from `philosophy.md` (lives in CLAUDE.md only)
...
- [ ] `doc_freshness_check.md` updated to include `philosophy.md` (Table 2, item M)
