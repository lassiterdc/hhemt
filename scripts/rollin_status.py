...
SPECS = [
    ...
    (
        "S8e",
        "S8",
        "$AW/library/docs/stipulations/hhemt/coupled resume validity warning fires on pre-fix triton with a resumed coupled sim.md",
        "The three invalidity arms",
    ),
    ...
    (
        "N5b",
        "N5",
        "src/hhemt/report_templates/captions/cross_experiment_intercomparison.rst",
        "Row counts differ per model by construction",
    ),
    ...
    (
        "F2d",
        "F2",
        "$AW/library/prompts/instructions/protocols/worktree aware project testing.md",
        "RESOLVED. The objection was that a bare interpreter",
    ),
    (
        "S4",
        "S4",
        "src/hhemt/report_renderers/cross_experiment_errors_and_warnings.py",
...
CLUSTER_TESTS = {
    ...
    "F4": [
        "test_eda_calc_ran_fails_when_targets_are_enumerated_but_no_verdicts_exist",
        "test_harvest_records_the_absent_source",
    ],
...
def main() -> int:
    ...
    for spec, cluster, target, marker in SPECS:
        ...
        applied += ok
        _state = "APPLIED" if ok else ("MISSING FILE" if n is None else "not applied")
