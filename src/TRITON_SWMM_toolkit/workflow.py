...
'''
    ...
    def generate_snakefile_content(
        ...
        # Onsuccess auto-render — fires render_report_runner per requested format
        # so analysis.run() produces the report without a separate render_report() call.
        ...
        render_cmds = "".join(
            ...
            f"            {config_args} \\\n"
            f"            --format {fmt} \\\n"
            f"            > {log_dir_str}/render_report_{fmt}.log 2>&1 || true"
            for fmt in _formats
        )
...
onsuccess:
    shell("""
        {self.python_executable} -m TRITON_SWMM_toolkit.export_scenario_status \\
            {config_args} \\
            > {log_dir_str}/export_scenario_status.log 2>&1{render_cmds}
    """)
...
class SensitivityAnalysisWorkflowBuilder:
    ...
    def generate_master_snakefile_content(
        ...
        # Onsuccess auto-render — fires render_report_runner per requested format
        # so analysis.run() produces the master report without a separate
        # render_report() call. Default ["zip"] keeps the single-file HTML
        # opt-in (it's 40+ MB; zip is the unbundled tree).
        _formats = report_formats if report_formats is not None else ["zip"]
        master_render_cmds = "".join(
            f"\n        {self.python_executable} -m TRITON_SWMM_toolkit.render_report_runner \\\n"
            f"            {master_config_args} \\\n"
            f"            --format {fmt} \\\n"
            f"            > {log_dir_str}/render_report_{fmt}.log 2>&1 || true"
            for fmt in _formats
        )
...
onsuccess:
    shell("""
        {self.python_executable} -m TRITON_SWMM_toolkit.export_scenario_status \\
            {master_config_args} \\
            > {log_dir_str}/export_scenario_status.log 2>&1{master_render_cmds}
    """)
