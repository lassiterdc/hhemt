...
"""
            ...
            # Write the internal workflow script (executed inside tmux)
            tmux_workflow_script = self.analysis_paths.analysis_dir / "logs" / "tmux_workflow.sh"
            tmux_workflow_script.parent.mkdir(parents=True, exist_ok=True)
            tmux_workflow_script.write_text(snakemake_cmd)
            tmux_workflow_script.chmod(0o755)  # Make executable

            # Create standalone launcher script (like the old run_workflow_batch_job.sh)
            # This can be executed manually for testing
            launcher_script = self.analysis_paths.analysis_dir / "run_workflow_tmux.sh"
            launcher_content = f"""#!/bin/bash
# Standalone tmux workflow launcher
# Generated by TRITON-SWMM toolkit
# Usage: ./run_workflow_tmux.sh

set -e

ANALYSIS_DIR="{self.analysis_paths.analysis_dir}"
SESSION_NAME="{session_name}"
WORKFLOW_SCRIPT="$ANALYSIS_DIR/logs/tmux_workflow.sh"

echo "=== TRITON-SWMM Tmux Workflow Launcher ==="
...
# Execute workflow script in the session
echo "Launching workflow in session..."
tmux send-keys -t "$SESSION_NAME" "bash $WORKFLOW_SCRIPT" Enter
...
echo "View logs: tail -f $ANALYSIS_DIR/logs/tmux_session.log"
"""
            launcher_script.write_text(launcher_content)
            launcher_script.chmod(0o755)

            if verbose:
                print(f"[Snakemake] Workflow script: {tmux_workflow_script}", flush=True)
                print(f"[Snakemake] Launcher script: {launcher_script}", flush=True)
                print(f"[Snakemake] You can test manually with: {launcher_script}", flush=True)
            ...
            # Execute the script in the tmux session instead of sending keys
            # This is much more reliable for multi-line commands
            exec_cmd = f"bash {tmux_workflow_script}"
            send_keys_cmd = f"{module_load_prefix}tmux send-keys -t {session_name} {shlex.quote(exec_cmd)} Enter"

            if verbose:
                print(f"[Snakemake] Executing script in session: {exec_cmd}", flush=True)
