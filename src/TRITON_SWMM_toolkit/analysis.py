...
class TRITONSWMM_analysis:
    ...
    def _generate_snakefile_content(
        ...
        sim_ids = ",".join(str(i) for i in range(n_sims))
        ...
        snakefile_content = f'''# Snakefile for TRITON-SWMM workflow
# Auto-generated by TRITONSWMM_analysis
# Uses Snakemake best practices with conda environments
...
rule all:
    input: "workflow_complete.flag"

rule setup:
    output: "setup_complete.flag"
    log: "logs/setup.log"
    conda: "{conda_env_path}"
    shell:
        """
        {self._python_executable} -m TRITON_SWMM_toolkit.setup_workflow \\
...
rule simulation:
    input: "setup_complete.flag"
    output: "sims/sim_{{event_iloc}}/complete.flag"
...
rule consolidate:
    input: expand("sims/sim_{{sim_id}}/complete.flag", sim_id=SIM_IDS)
    output: "workflow_complete.flag"
...
'''
    ...
    def submit_workflow(
        ...
        pickup_where_leftoff : bool
            If True, resume simulations from last checkpoint
        consolidate_outputs : bool
            If True, consolidate outputs after all simulations complete
        verbose : bool
            If True, print progress messages
        ...
        if mode == "auto":
            mode = "slurm" if self.in_slurm else "local"
        ...
        if mode == "local":
            result = self._run_snakemake_local(
    ...
    def _run_snakemake_slurm(
        ...
        try:
            ...
            if verbose:
                if job_id:
                    print(
                        f"[Snakemake] Workflow submitted with job ID: {job_id}",
                        flush=True,
                    )
