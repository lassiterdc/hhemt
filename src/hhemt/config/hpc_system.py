...
class hpc_system_config(BaseModel):
    ...
    max_concurrent_cpus: int | None = Field(
        ...
        description=(
            ...
            "ABSENT, not infinite -- snakemake's scheduler iterates `for name in "
            "self.global_resources` (job_scheduler.py:614-620) and frees on `if name in "
            "self.resources` (:518-521), so an undeclared budget name is not in the loop at all "
