    ...
    def from_directory(cls, path: Path | str) -> Bundle:
        ...
        if bundle_version < BUNDLE_SCHEMA_VERSION:
            raise BundleSchemaError(
                ...
                f"evidence that the cfg is wrong. Re-emit the bundle from its "
                f"source analysis with the current toolkit. Per generation: "
                f"v1->v2 made `report:` required and dropped the legacy "
                f"report-config peer-file, so a v1 cfg fails on the MISSING key "
                f"(that transition also needs V0005 run on the source dir first); "
                f"v2->v3 added Workflow-Run-Crate typing and reprex carriage; "
                f"v3->v4 renamed reclaim_after_processing to "
                f"remove_after_processing, so a v3 cfg fails on the EXTRA "
                f"retired key."
            )
