...
Usage:
    python -m hhemt.version_migration migrate {dir} \
        [--target N] [--apply]
    python -m hhemt.version_migration status {dir}
    python -m hhemt.version_migration baseline {dir} {version} \
        [--force]
    python -m hhemt.version_migration verify {dir}
"""
...
app = typer.Typer(help="hhemt version-migration CLI")
...
def _classify_for_dry_run(analysis_dir: Path) -> tuple[str, str]:
    ...
    when RECOVERABLE)."""
    from hhemt.version_migration.versions.V0005__inline_report_config import (
